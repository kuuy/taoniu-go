...
type Trigger struct {
  ...
  Status       int       `gorm:"not null;index:idx_binance_futures_tradings_triggers_updated_status,priority:2"`
  Version      int       `gorm:"not null"`
  Remark       string    `gorm:"size:5000;not null"`
  CreatedAt    time.Time `gorm:"not null"`
  UpdatedAt    time.Time `gorm:"not null;index;index:idx_binance_futures_tradings_triggers_updated_status,priority:1"`
}

func (m *Trigger) TableName() string {
  return "binance_futures_tradings_triggers"
}
