...
type Kline struct {
  ...
  Symbol    string    `gorm:"size:20;not null;uniqueIndex:unq_raydium_perpetuals_klines"`
  Interval  string    `gorm:"size:3;not null;uniqueIndex:unq_raydium_perpetuals_klines"`
  ...
  Timestamp int64     `gorm:"not null;uniqueIndex:unq_raydium_perpetuals_klines"`
  CreatedAt time.Time `gorm:"not null"`
  UpdatedAt time.Time `gorm:"not null"`
}

func (m *Kline) TableName() string {
  return "raydium_perpetuals_klines"
}
