...
func (h *Orders) Open(symbol string) (*asynq.Task, error) {
  ...
  return asynq.NewTask("binance:margin:cross:orders:open", payload), nil
}
...
func (h *Orders) Flush(symbol string, orderId int64) (*asynq.Task, error) {
  ...
  return asynq.NewTask("binance:margin:cross:orders:flush", payload), nil
}
...
func (h *Orders) Sync(symbol string, startTime int64, limit int) (*asynq.Task, error) {
  ...
  return asynq.NewTask("binance:margin:cross:orders:sync", payload), nil
}
