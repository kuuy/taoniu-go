...
func (h *Triggers) Flush(symbol string) (*asynq.Task, error) {
  ...
  return asynq.NewTask("binance:spot:margin:cross:tradings:triggers:flush", payload), nil
}
...
func (h *Triggers) Place(symbol string) (*asynq.Task, error) {
  ...
  return asynq.NewTask("binance:spot:margin:cross:tradings:triggers:place", payload), nil
}
