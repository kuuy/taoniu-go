...
func (h *Orders) Register() error {
  h.AnsqContext.Mux.HandleFunc("binance:margin:cross:orders:open", h.Open)
  h.AnsqContext.Mux.HandleFunc("binance:margin:cross:orders:flush", h.Flush)
  h.AnsqContext.Mux.HandleFunc("binance:margin:cross:orders:sync", h.Sync)
  return nil
}
