...
func (h *Minutely) Atr(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1m:atr"),
  )
...
func (h *Minutely) Zlema(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1m:zlema"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Minutely) HaZlema(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1m:hazlema"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Minutely) Kdj(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1m:kdj"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Minutely) BBands(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1m:bbands"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Minutely) Register(mux *asynq.ServeMux) error {
  mux.HandleFunc("binance:spot:strategies:1m:atr", h.Atr)
  mux.HandleFunc("binance:spot:strategies:1m:zlema", h.Zlema)
  mux.HandleFunc("binance:spot:strategies:1m:hazlema", h.HaZlema)
  mux.HandleFunc("binance:spot:strategies:1m:kdj", h.Kdj)
  mux.HandleFunc("binance:spot:strategies:1m:bbands", h.BBands)
  return nil
}
