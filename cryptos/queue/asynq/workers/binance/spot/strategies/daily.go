...
func (h *Daily) Atr(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1d:atr"),
  )
...
func (h *Daily) Zlema(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1d:zlema"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Daily) HaZlema(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1d:hazlema"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Daily) Kdj(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1d:kdj"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Daily) BBands(ctx context.Context, t *asynq.Task) error {
  ...
  mutex := common.NewMutex(
    h.Rdb,
    h.Ctx,
    fmt.Sprintf("locks:binance:spot:strategies:1d:bbands"),
  )
  if !mutex.Lock(30 * time.Second) {
    return nil
  }
...
func (h *Daily) Register(mux *asynq.ServeMux) error {
  mux.HandleFunc("binance:spot:strategies:1d:atr", h.Atr)
  mux.HandleFunc("binance:spot:strategies:1d:zlema", h.Zlema)
  mux.HandleFunc("binance:spot:strategies:1d:hazlema", h.HaZlema)
  mux.HandleFunc("binance:spot:strategies:1d:kdj", h.Kdj)
  mux.HandleFunc("binance:spot:strategies:1d:bbands", h.BBands)
  return nil
}
