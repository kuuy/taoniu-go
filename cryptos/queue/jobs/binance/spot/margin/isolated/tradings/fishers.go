...
func (h *Fishers) Flush(symbol string) (*asynq.Task, error) {
	...
	return asynq.NewTask("binance:spot:margin:isolated:tradings:fishers:flush", payload), nil
}
...
func (h *Fishers) Place(symbol string) (*asynq.Task, error) {
	...
	return asynq.NewTask("binance:spot:margin:isolated:tradings:fishers:place", payload), nil
}
