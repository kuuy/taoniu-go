...
func (r *MinutelyRepository) Gets(symbols []string, fields []string) []string {
  var script = redis.NewScript(`
	...
	local data = {}
	for i = 1, #KEYS do
		local key = 'binance:futures:indicators:1m:' .. KEYS[i]
...
func (r *MinutelyRepository) Ranking(
  symbol string,
  fields []string,
  sortField string,
  sortType int,
...
func (r *MinutelyRepository) Pivot(symbol string) error {
...
func (r *MinutelyRepository) Atr(symbol string, period int, limit int) error {
  ...
  for _, item := range klines {
    if timestamp > 0 && (timestamp-item.Timestamp) != 60000 {
      return nil
    }
...
func (r *MinutelyRepository) Zlema(symbol string, period int, limit int) error {
  var klines []*models.Kline
  r.Db.Select(
...
func (r *MinutelyRepository) HaZlema(symbol string, period int, limit int) error {
  var klines []*models.Kline
  r.Db.Select(
...
func (r *MinutelyRepository) Kdj(symbol string, longPeriod int, shortPeriod int, limit int) error {
  var klines []*models.Kline
  r.Db.Select(
    []string{"open", "close", "high", "low", "timestamp"},
  ).Where(
...
func (r *MinutelyRepository) BBands(symbol string, period int, limit int) error {
  var klines []*models.Kline
  r.Db.Select(
    []string{"open", "close", "high", "low", "timestamp"},
  ).Where(
...
func (r *MinutelyRepository) VolumeProfile(symbol string, limit int) error {
...
func (r *MinutelyRepository) Day(timestamp int64) (day string, err error) {
  now := time.Now()
  _, offset := now.Zone()
