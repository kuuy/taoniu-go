...
func (r *AndeanOscillatorRepository) Flush(symbol string, interval string, period int, length int, limit int) (err error) {
  ...
  for i := 1; i < limit; i++ {
    ...
    // Upper envelope: EMA of max (decays downward when price drops)
    up1[i] = math.Max(closes[i], math.Max(opens[i], up1[i-1]-alpha*(up1[i-1]-math.Min(closes[i], opens[i]))))
    up2[i] = math.Max(close2, math.Max(open2, up2[i-1]-alpha*(up2[i-1]-math.Min(close2, open2))))

    // Lower envelope: EMA of min (decays upward when price rises)
    dn1[i] = math.Min(closes[i], math.Min(opens[i], dn1[i-1]+alpha*(math.Max(closes[i], opens[i])-dn1[i-1])))
    dn2[i] = math.Min(close2, math.Min(open2, dn2[i-1]+alpha*(math.Max(close2, open2)-dn2[i-1])))
