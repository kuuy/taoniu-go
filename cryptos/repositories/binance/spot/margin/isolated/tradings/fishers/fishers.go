...
func (r *FishersRepository) Scan() []string {
	var symbols []string
	r.Db.Model(&isolatedModels.Fisher{}).Where("status", []int{1, 3}).Distinct().Pluck("symbol", &symbols)
	return symbols
}
...
func (r *FishersRepository) Flush(symbol string) error {
	...
	for _, grid := range grids {
		if grid.Status == 0 {
			...
			if status == "" || status == "NEW" || status == "PARTIALLY_FILLED" {
				r.OrdersRepository.Flush(symbol, grid.SellOrderId, true)
				continue
			}
			r.Db.Transaction(func(tx *gorm.DB) error {
				if status == "FILLED" {
...
func (r *FishersRepository) Take(fisher *isolatedModels.Fisher, price float64) error {
	var grid models.Grid
