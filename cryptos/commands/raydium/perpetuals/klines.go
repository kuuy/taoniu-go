...
import (
	...
	"taoniu.local/cryptos/repositories/raydium/perpetuals"
)
...
type KlinesHandler struct {
	...
	Rdb              *redis.Client
	Ctx              context.Context
	KlinesRepository *perpetuals.KlinesRepository
}
...
func NewKlinesCommand() *cli.Command {
	...
	return &cli.Command{
		...
		Before: func(c *cli.Context) error {
			...
			h.KlinesRepository = &perpetuals.KlinesRepository{
				Db:  h.Db,
				Rdb: h.Rdb,
				Ctx: h.Ctx,
			}
...
func (h *KlinesHandler) Flush(symbol string, interval string, limit int) (err error) {
	log.Println("raydium perpetuals klines flush...", symbol, interval, limit)
	raydium.NewPerpetuals().AutoMigrate(h.Db)
	endTime := h.KlinesRepository.Timestamp(interval)
	startTime := endTime - int64(limit)*h.KlinesRepository.Timestep(interval)
	err = h.KlinesRepository.Flush(symbol, interval, startTime, endTime)
	if err != nil {
