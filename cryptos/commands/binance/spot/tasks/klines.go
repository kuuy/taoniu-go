...
func (h *KlinesHandler) Flush(interval string, current int) error {
  log.Printf("flushing spot klines [%s] (batch %d)...", interval, current)
...
func (h *KlinesHandler) Fix(interval string, current int) error {
  log.Printf("fixing spot klines [%s] (batch %d)...", interval, current)
  symbols := h.ScalpingRepository.Scan()
...
func (h *KlinesHandler) Clean() error {
  log.Println("cleaning old spot klines...")
  symbols := h.ScalpingRepository.Scan()
