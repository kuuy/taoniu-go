...
func (r *BridgesRepository) Count(status []int) (int64, error) {
	var count int64
	r.Db.Model(&models.Bridge{}).Where("status", status).Count(&count)
	return count, nil
}

func (r *BridgesRepository) Random(i int, limit int) ([]string, error) {
	count, _ := r.Count([]int{0, 1, 3})
	...
	for _, item := range items {
		r.Rdb.ZRem(r.Ctx, "tor:bridges", item).Result()
	}

	var ids []string
	var bridges []string
	for len(bridges) < limit {
		items, _ := r.Rdb.ZRevRange(r.Ctx, "tor:bridges", 0, -1).Result()
