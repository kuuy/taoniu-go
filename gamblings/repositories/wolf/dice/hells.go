...
func (r *HellsRepository) Apply(currency string) error {
  ...
  if errors.Is(result.Error, gorm.ErrRecordNotFound) {
    ...
    rnd.Shuffle(len(rules), func(i, j int) { rules[i], rules[j] = rules[j], rules[i] })

    rule := rules[(rnd.Intn(313-13)+13)%len(rules)]
    multiplier := 4.95
    ...
    if stopBalance < balance-10 {
      ...
      targetBalance = math.Round(balance*10000000000+9090909090910*0.000008) / 10000000000
